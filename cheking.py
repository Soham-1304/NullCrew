...
def call_gemini_api(user_input):
    ...
    try:
        ...
        return response.text  # Access the text property of the response
...
def main():
    ...
    binary_array = generate_1d_binary_array(active_keywords, main_keywords)
    print("\nGenerated Binary Vector (first 30 bits):")
    print(binary_array)
    print("... (total length: {})".format(len(binary_array)))
    
    # Next, you would convert binary_vector to a numpy array, scale it if needed, and feed it to your ML model.
    # For example:
    #   import numpy as np
    #   vector_np = np.array(binary_vector).reshape(1, -1)
    #   vector_scaled = scaler.transform(vector_np)
    #   predicted = model.predict(vector_scaled)
    #   predicted_disease = label_encoder.inverse_transform(np.argmax(predicted, axis=1))[0]
    # And then use Gemini to generate follow-up questions based on predicted_disease.
